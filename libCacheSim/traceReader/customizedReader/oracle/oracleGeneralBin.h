...
static inline int oracleGeneralBinFlows_setup(reader_t *reader) {
  reader->trace_type = ORACLE_GENERAL_TRACE_FLOWS;
  reader->trace_format = BINARY_TRACE_FORMAT;
  reader->item_size = 32;
  reader->n_total_req = (uint64_t)reader->file_size / (reader->item_size);
  printf("n_total_req: %ld\n", reader->n_total_req);
  reader->obj_id_is_num = true;
  return 0;
}
...
static inline int oracleGeneralBinFlows_read_one_req(reader_t *reader,
                                                request_t *req) {
  char *record = read_bytes(reader);
  ...
  req->clock_time = *(uint32_t *)record;
  req->obj_id = *(uint64_t *)(record + 8);
  req->obj_size = *(uint32_t *)(record + 16);
  req->next_access_vtime = *(int64_t *)(record + 24);
  if (req->next_access_vtime == -1) {
    req->next_access_vtime = INT64_MAX;
  }
